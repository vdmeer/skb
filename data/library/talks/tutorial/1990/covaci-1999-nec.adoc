_Management of Broadband Networks II_, 1999

* Presenters: Stefan Covaci, Ingo Busse, Sven van der Meer, Stefan Arbanowski
* Tutorial: NEC Europe Ltd., C&C Research Laboratory, Berlin, Germany, May 27-28, 1999
* Links:
      skb:
        link:https://github.com/vdmeer/skb/tree/master/data/library/talks/tutorial/1990/covaci-1999-nec.yaml[yaml],
        link:https://github.com/vdmeer/skb/tree/master/data/library/talks/tutorial/1990/covaci-1999-nec.adoc[adoc]
ifdef::local[]
    ┃ local:
        link:library/talks/tutorial/1990/[Folder]
endif::[]

