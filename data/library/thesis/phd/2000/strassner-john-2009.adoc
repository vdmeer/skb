*John C. Strassner*: _Enabling Autonomic Network Management Decisions Using a Novel Semantic Representation and Reasoning Approach_, 2009

* Author: John C. Strassner
* Thesis (PhD): Waterford Institute of Technology (WIT), Waterford, Ireland
* Links:
      skb:
        link:https://github.com/vdmeer/skb/tree/master/data/library/thesis/phd/2000/strassner-john-2009.yaml[yaml],
        link:https://github.com/vdmeer/skb/tree/master/data/library/thesis/phd/2000/strassner-john-2009.adoc[adoc]
ifdef::local[]
    ┃ local:
        link:library/thesis/phd/2000/[Folder]
endif::[]

